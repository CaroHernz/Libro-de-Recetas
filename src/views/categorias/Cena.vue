<template>
  <div class="cena">
    <div class="container py-4">
      <h2 class="text-center display-4 text-primary mb-4">Recetas de Cena</h2>
      ...
      <div class="recetas-menu mb-4" v-if="$route.params.id && recetas.length">
        <div class="d-flex flex-nowrap overflow-auto justify-content-center">
          <button 
            ...
            :class="['btn', 'rounded-pill','border-0','btn-sm','me-2', receta.id === recetaActualId ? 'btn-primary' : 'btn-outline-primary']"
          >
            {{ receta.nombre }}
          </button>
        </div>
...
export default {
  ...
  methods: {
    cargarRecetas() {
      const categoriaData = recetas.find(c => c.hasOwnProperty('cena'))
      this.recetas = categoriaData ? categoriaData.cena : []
    ...
    irAReceta(id) {
      this.$router.push(`/categorias/cena/receta/${id}`)
    },
    navReceta(direccion) {
      ...
      if(nuevoIndice >= 0 && nuevoIndice < this.recetas.length) {
        ...
        this.$router.push(`/categorias/cena/receta/${nuevaReceta.id}`)
      }
    }
  }
}
