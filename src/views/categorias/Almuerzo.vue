<template>
  <div class="almuerzo">
    <div class="container py-4">
      <h2 class="text-center display-4 text-danger mb-4">Recetas de Almuerzo</h2>
      ...
      <div class="recetas-menu mb-4" v-if="$route.params.id && recetas.length">
        <div class="d-flex flex-nowrap overflow-auto justify-content-center">
          <button 
            ...
            :class="['btn','btn-sm','rounded-pill','border-0','me-2', receta.id === recetaActualId ? 'btn-danger' : 'btn-outline-danger']"
          >
            {{ receta.nombre }}
          </button>
        </div>
...
export default {
  ...
  methods: {
    cargarRecetas() {
      const categoriaData = recetas.find(c => c.hasOwnProperty('almuerzo'))
      this.recetas = categoriaData ? categoriaData.almuerzo : []
    ...
    irAReceta(id) {
      this.$router.push(`/categorias/almuerzo/receta/${id}`)
    },
    navReceta(direccion) {
      ...
      if(nuevoIndice >= 0 && nuevoIndice < this.recetas.length) {
        ...
        this.$router.push(`/categorias/almuerzo/receta/${nuevaReceta.id}`)
      }
    }
  }
}
...
.recetas-menu {
  background: linear-gradient(to right, rgba(255, 7, 7, 0.1), rgba(255, 7, 7, 0.05));
  padding: 1rem;
  border-radius: 10px;
}
</style>