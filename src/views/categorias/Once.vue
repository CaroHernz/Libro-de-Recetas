<template>
  <div class="once">
    <div class="container py-4">
      <h2 class="text-center display-4 text-success mb-4">Recetas de Once</h2>
      ...
      <div class="recetas-menu mb-4" v-if="$route.params.id && recetas.length">
        <div class="d-flex flex-nowrap overflow-auto justify-content-center">
          <button 
            ...
            :class="['btn', 'btn-sm', 'rounded-pill','border-0','me-2', receta.id === recetaActualId ? 'btn-success' : 'btn-outline-success']"
          >
            {{ receta.nombre }}
          </button>
        </div>
...
export default {
  ...
  methods: {
    cargarRecetas() {
      const categoriaData = recetas.find(c => c.hasOwnProperty('once'))
      this.recetas = categoriaData ? categoriaData.once : []
      this.recetas.forEach(receta => {
        if(!receta.categoria) {
          receta.categoria='once'
        }
      })
    },
    irAReceta(id) {
      this.$router.push(`/categorias/once/receta/${id}`)
    },
    navReceta(direccion) {
      ...
      if(nuevoIndice >= 0 && nuevoIndice < this.recetas.length) {
        ...
        this.$router.push(`/categorias/once/receta/${nuevaReceta.id}`)
      }
    }
  }
}
...
.recetas-menu {
  background: linear-gradient(to right, rgba(25, 135, 84, 0.1), rgba(25, 135, 84, 0.05));
  padding: 1rem;
  border-radius: 10px;
}
</style>